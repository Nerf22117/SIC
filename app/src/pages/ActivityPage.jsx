...
} from "../graphql/queries/exercise.query.js";
...
export default function ActivityPage() {
  ...
  const {
    data: dataUserFood,
    loading: loadingUserFood,
    error: errorUserFood,
    refetch: refetchUserFood,
  } = useQuery(GET_USER_FOODS, {
    variables: {
      input: {
        userId: authUser?._id,
  ...
  const {
    data: dataUserExercise,
    loading: loadingUserExercise,
    error: errorUserExercise,
    refetch: refetchUserExercise,
  } = useQuery(GET_USER_EXERCISES, {
    variables: {
      input: {
        userId: authUser?._id,
        startDate: startDate?.toISOString().split("T")[0],
      ...
      <div className="flex flex-col md:flex-row md:items-center md:space-x-4 mb-2">
        <div className="flex flex-col md:flex-row md:items-center md:space-x-4 mb-2">
          <DatePicker
            ...
            className="border border-gray-300 p-2 w-full md:w-1/2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <DatePicker
            selected={endDate}
      ...
      <div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:space-x-4 mb-2">
          ...
          >
            ...
            {dataMuscularGroups?.getUserExerciseCategory?.result?.map(
              (group) => (
          ...
          {dataUserExercise?.getUserExercises?.result?.length > 0 ? (
            dataUserExercise.getUserExercises.result.map((exercise) => (
              ...
              >
                ...
                <p className="text-gray-500 text-sm">
                  Muscular Group: {exercise.muscularGroup}
                </p>
              </div>
            ))
          ) : (
